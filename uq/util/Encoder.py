...
Adapted from festim_niuq for OpenMC models.
"""
import os
import yaml
import math
...
class AdvancedYAMLEncoder(YAMLEncoder):
    ...
    def __init__(self, template_fname, target_filename="config.yaml",
        ...
        parameter_map : dict
            Mapping of parameter names to YAML paths (e.g., {"enrichment": "materials.fuel.enrichment.mean"})
        type_conversions : dict
            Type conversions for parameters (e.g., {"enrichment": float})
        fixed_parameters : dict
            Parameters that are fixed for every run (not varied by EasyVVUQ)
        """
        self.template_fname = template_fname
        self.target_filename = target_filename
    ...
    def encode(self, params=None, target_dir="./"):
        ...
        for param_name, param_value in params.items():
            print(f" >> Encoding parameter '{param_name}' with value '{param_value}'")
    ...
    def _set_nested_value(self, config, path, value):
        ...
        if isinstance(value, float) and not math.isclose(current[keys[-1]], value):
            raise ValueError(
                f"Failed to set value at {path}. Expected {value}, got {current[keys[-1]]}"
            )
