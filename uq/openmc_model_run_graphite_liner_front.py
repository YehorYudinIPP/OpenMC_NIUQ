#!/usr/bin/env python3
"""
openmc_model_run_graphite_liner_front.py – OpenMC TBM model with graphite liner
on the front (neutron-facing) wall.

Variant of ``openmc_model_run_graphite_liner.py`` where the thin graphite layer
is placed on the **inner side of the front wall** of the TBM casing, i.e.
between the Eurofer-97 front wall and the Li2TiO3 ceramic (or pebble bed)
region.  Neutrons enter through the graphite before reaching the ceramic, which
pre-thermalizes them and can boost the Li-6(n,t) reaction rate.

The ceramic volume is reduced by the liner thickness along the z-axis
(same total TBM dimensions as the base model).

All other geometry dimensions, materials, source definition, and tallies are
identical to the base model.

Usage
-----
    python openmc_model_run_graphite_liner_front.py --config config.yaml
"""
...
def build_openmc_model(config):
    ...
    This is the **front graphite-liner variant**: the inner side of the TBM
    front wall has an additional graphite layer (default 0.5 cm) between the
    Eurofer-97 casing and the ceramic fill.  Neutrons pass through the graphite
    first, which pre-thermalizes them before they enter the Li2TiO3 breeder.
    ...
    Geometry (from the problem statement notebook):
      ...
      * Test Blanket Module (TBM): Eurofer-97 casing filled with Li2TiO3
        ...
        of the front wall (neutron-facing side).
    ...
    # Graphite liner on the inner front wall of the TBM (cm)
    ...
    # TBM inner surfaces (full inner cavity, excluding Eurofer walls)
    ...
    # Graphite liner surface (inner face of the FRONT wall)
    # The liner occupies inner_start .. graphite_liner_end;
    # the ceramic occupies graphite_liner_end .. inner_end.
    graphite_liner_end = openmc.ZPlane(
        z0=base_case + eurofer_thickness + graphite_liner_thickness)
    ...
    # TBM regions
    # Graphite liner occupies from inner_start to graphite_liner_end (front side)
    graphite_liner_region = (+inner_left & -inner_right & +inner_bottom & -inner_top
                             & +inner_start & -graphite_liner_end)
    # Ceramic fill occupies from graphite_liner_end to inner_end
    inner_region  = (+inner_left & -inner_right & +inner_bottom & -inner_top
                     & +graphite_liner_end & -inner_end)
...
def _get_mean(value):
    ...
    1. Add it to ``model_config_graphite_liner_front.yaml`` with
       ``mean`` / ``relative_stdev`` / ``pdf`` keys (or as a plain scalar
...
def main():
    print("\n ! Entering OpenMC EasyVVUQ model wrapper "
          "(TBM pebble-bed, graphite liner – front wall) !\n")
